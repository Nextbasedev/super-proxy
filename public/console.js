...
// flow keys like 'nextbase.paradox') are rendered verbatim, no fake ID glyph.
...
const ROUTES = {
  ...
  setup:    { title: 'Setup',     subtitle: 'Wire OpenClaw (and other clients) to this gateway', adminOnly: false, render: () => renderSetup() },
...
const NAV_ADMIN = [
  ...
  { route: 'setup',    label: 'Setup',      icon: 'plug' },
];
const NAV_DEV = [
...
async function openAccountDrawer(a) {
  ...
  const body = el('div', { class: 'field-group' },
    ...
    el('div', { class: 'field' },
      ...
      el('textarea', { id: 'acct_notes', placeholder: 'Internal notes…', on: { input: (e) => editNotes = e.target.value } }, editNotes),
    ),
    el('div', { class: 'field-row' },
      el('div', { class: 'field' },
...
function openAddAccountApiKey(providerHint) {
  ...
  const body = el('div', { class: 'field-group' },
    ...
    el('div', { class: 'field-row' },
      el('div', { class: 'field' },
        el('label', { class: 'field-label' }, 'Label'),
        el('input', { placeholder: 'e.g. nextbase.app', on: { input: (e) => label = e.target.value } }),
      ),
      el('div', { class: 'field' },
...
function openCreateUser() {
  ...
  const body = el('div', { class: 'field-group' },
    el('div', { class: 'field' },
      ...
      el('input', { type: 'email', placeholder: 'someone@nextbase.app', on: { input: (e) => email = e.target.value } }),
    ),
    el('div', { class: 'field' },
...
/* ══════════════════════════════════════════════════════════════════
   ▓▓▓▓▓ SETUP PAGE  ▓▓▓▓▓
   "Set up OCPlatform in 3 steps" hero + provider tabs. The selected
   token (raw sp_* if known this session, else <YOUR_TOKEN>) flows
   into every snippet on the page. Visible to admins + developers.
   ══════════════════════════════════════════════════════════════════ */

// In-memory + sessionStorage map of token id → raw sp_* secret.
// Populated only when the user creates a token in *this* tab; the
// server never re-returns the raw token after issuance, so any token
// created in another session stays as <YOUR_TOKEN>.
const RAW_TOKEN_STORE_KEY = 'sp.rawTokens.v1';
const setupRawTokens = (() => {
  try {
    const raw = sessionStorage.getItem(RAW_TOKEN_STORE_KEY);
    return raw ? JSON.parse(raw) : {};
  } catch {
    return {};
  }
})();
function rememberRawToken(id, raw) {
  if (!id || !raw) return;
  setupRawTokens[String(id)] = raw;
  try { sessionStorage.setItem(RAW_TOKEN_STORE_KEY, JSON.stringify(setupRawTokens)); } catch {}
}
function rawTokenFor(id) {
  if (id == null) return null;
  return setupRawTokens[String(id)] || null;
}
let setupPreferredTokenId = null;
...
function currentSetupTab() {
  const m = /(?:^|&)setup=([a-z]+)/.exec(location.hash || '');
  const id = m && m[1];
  return SETUP_TABS.find(t => t.id === id)?.id || 'anthropic';
}
function setSetupTab(id) {
  const hash = (location.hash || '').replace(/^#/, '');
  const parts = hash.split('&').filter(p => p && !p.startsWith('setup='));
  parts.push(`setup=${id}`);
  location.hash = parts.join('&');
}

// Client axis (OCPlatform vs Hermes). The provider list is identical; only the
// wiring differs. Persisted in the hash like the tab state.
const SETUP_CLIENTS = [
  { id: 'openclaw', label: 'OCPlatform' },
  { id: 'hermes',   label: 'Hermes' },
];
function currentSetupClient() {
  const m = /(?:^|&)client=([a-z]+)/.exec(location.hash || '');
  const id = m && m[1];
  return SETUP_CLIENTS.find(c => c.id === id)?.id || 'openclaw';
}
function setSetupClient(id) {
  const hash = (location.hash || '').replace(/^#/, '');
  const parts = hash.split('&').filter(p => p && !p.startsWith('client='));
  parts.push(`client=${id}`);
  location.hash = parts.join('&');
}

// Hermes custom_providers descriptors — mirror the live config.yaml shape.
// suffix appended to baseUrl; apiMode is the Hermes wire-protocol selector.
const HERMES_PROVIDERS = {
  anthropic:  { name: 'nextbase-anthropic',  suffix: '',             apiMode: 'anthropic_messages', model: 'claude-opus-4-6',                models: ['claude-opus-4-6', 'claude-sonnet-4-5-20250929', 'claude-haiku-4-5'] },
  codex:      { name: 'nextbase-codex',      suffix: '/v1',          apiMode: 'codex_responses',    model: 'gpt-5.5',                       models: ['gpt-5.5', 'gpt-5.4', 'gpt-5.3-codex'] },
  xai:        { name: 'nextbase-xai',        suffix: '/v1/xai',      apiMode: 'codex_responses',    model: 'grok-4.3',                      models: ['grok-4.3', 'grok-4', 'grok-4-fast'] },
  groq:       { name: 'nextbase-groq',       suffix: '/v1/groq',     apiMode: 'chat_completions',   model: 'openai/gpt-oss-120b',       models: ['openai/gpt-oss-120b', 'llama-3.1-8b-instant'] },
  cerebras:   { name: 'nextbase-cerebras',   suffix: '/v1/cerebras', apiMode: 'chat_completions',   model: 'qwen-3-235b-a22b-instruct-2507', models: ['qwen-3-235b-a22b-instruct-2507', 'gpt-oss-120b', 'zai-glm-4.7'] },
  kimi:       { name: 'nextbase-kimi',       suffix: '/v1/kimi',     apiMode: 'chat_completions',   model: 'k3',                          models: ['k3', 'kimi-k2.7-code', 'kimi-k2.6', 'kimi-for-coding'] },
  glm:        { name: 'nextbase-glm',        suffix: '/v1/glm',      apiMode: 'anthropic_messages', model: 'glm-5.2',                       models: ['glm-5.2', 'glm-5.1', 'glm-4.7', 'glm-4.6'] },
  openrouter: { name: 'nextbase-openrouter', suffix: '/v1/openrouter', apiMode: 'chat_completions', model: 'tencent/hy3:free',              models: ['tencent/hy3:free'] },
};
function hermesProviderYaml(p, baseUrl) {
  const modelsBlock = p.models.map(m => `    ${m}: {}`).join('\n');
  return `custom_providers:\n- name: ${p.name}\n  base_url: ${baseUrl}${p.suffix}\n  key_env: NEXTBASE_MODEL_GATEWAY_API_KEY\n  api_mode: ${p.apiMode}\n  model: ${p.model}\n  models:\n${modelsBlock}\n  discover_models: false`;
}
function hermesSetupBlock({ tabId, baseUrl, token, agentLine, verifyCurl, verifyNote }) {
  const p = HERMES_PROVIDERS[tabId];
  if (!p) {
    return el('div', { class: 'stack' },
      setupAgentCard(agentLine),
      el('p', { class: 'fs-13 text-muted', style: { lineHeight: '1.55' } },
        'This provider does not have a dedicated Hermes template yet. See the Hermes skill for the general custom_providers pattern.'),
      el('div', { class: 'setup-block-label mt-2' }, 'Hermes setup skill'),
      setupCodeBlock(`Read ${baseUrl}/skills/hermes/SKILL.md and set me up. My sp_* token is ${token}.`),
    );
  }
  const envLine = `echo 'NEXTBASE_MODEL_GATEWAY_API_KEY=${token}' >> ~/.hermes/.env`;
  return el('div', { class: 'stack' },
    setupAgentCard(agentLine),
    el('p', { class: 'fs-13 text-muted', style: { lineHeight: '1.55' } },
      'Hermes uses ~/.hermes/config.yaml (custom_providers) and reads the token from an env var — not inline. The Anthropic billing path is detected automatically.'),
    el('div', { class: 'setup-block-label' }, '1 · Token — add to ', el('code', { class: 'mono' }, '~/.hermes/.env')),
    setupCodeBlock(envLine),
    el('div', { class: 'setup-block-label mt-3' }, '2 · Provider — merge into ', el('code', { class: 'mono' }, '~/.hermes/config.yaml')),
    setupCodeBlock(hermesProviderYaml(p, baseUrl)),
    el('div', { class: 'setup-block-label mt-3' }, '3 · Restart Hermes, then verify'),
    verifyNote ? el('p', { class: 'fs-12 text-muted', style: { lineHeight: '1.5' } }, verifyNote) : null,
    verifyCurl ? setupCodeBlock(verifyCurl) : null,
  );
}

function renderSetup() {
  const root = el('div', { class: 'stack stack--lg setup-page' });
  const baseUrl = `${location.protocol}//${location.host}`;
  const tokens = (state.data.tokens || []).filter(t => t.enabled !== false && t.enabled !== 0);

  // Selected token id is module-local to this render call but survives
  // user actions (dropdown change, token creation) via paint().
  let activeTokenId = (setupPreferredTokenId != null && tokens.some(t => t.id === setupPreferredTokenId))
    ? setupPreferredTokenId
    : (tokens[0]?.id ?? null);
  setupPreferredTokenId = activeTokenId;

  // Track whether the raw token is currently revealed in the hero card.
  let revealed = false;
  // Latest token-check result (null = not run yet).
  let tokenCheckResult = null;

  function effectiveToken() {
    return activeTokenId != null ? rawTokenFor(activeTokenId) : null;
  }
  function tokenForSnippet() {
    return effectiveToken() || '<YOUR_TOKEN>';
  }

  // ─── Hero card ────────────────────────────────────────────────
  const heroMount = el('section', { class: 'setup-hero card', 'aria-label': 'Set up OpenClaw in 3 steps' });
  const tabsMount = el('section', { class: 'setup-tabs-wrap' });
  root.appendChild(heroMount);
  root.appendChild(tabsMount);

  function tokenSelector() {
    if (!tokens.length) {
      return el('div', { class: 'flex gap-2 items-center', style: { flexWrap: 'wrap' } },
        el('span', { class: 'fs-12 text-subtle' }, 'No active tokens yet.'),
        el('button', { class: 'btn btn--primary btn--sm', on: { click: createTokenInline } },
          icon('plus', 14), 'Create token'),
      );
    }
    const sel = el('select', {
      class: 'input setup-token-select',
      'aria-label': 'Active token',
      on: { change: (e) => { activeTokenId = Number(e.target.value); setupPreferredTokenId = activeTokenId; revealed = false; tokenCheckResult = null; paint(); } },
    },
      ...tokens.map(t => {
        const has = rawTokenFor(t.id) ? ' ✓' : '';
        return el('option', { value: t.id, ...(t.id === activeTokenId ? { selected: 'selected' } : {}) },
          `${t.label || 'token'} · ${t.token_prefix || ''}…${has}`);
      })
    );
    return el('div', { class: 'flex gap-2 items-center setup-token-row', style: { flexWrap: 'wrap' } },
      sel,
      el('button', { class: 'btn btn--ghost btn--sm', on: { click: createTokenInline } },
  ...
  async function createTokenInline() {
    if (isAdminUser()) {
      // Reuse the admin issue-token drawer for the currently selected user
      // (the one who owns the active token) or for the admin themselves.
      const meEmail = state.user?.email;
      let targetUser = null;
      if (activeTokenId != null) {
        const t = tokens.find(x => x.id === activeTokenId);
        if (t && Array.isArray(state.data.users)) {
          targetUser = state.data.users.find(u => u.id === t.user_id || u.email === t.user_email) || null;
        }
      }
      if (!targetUser && Array.isArray(state.data.users)) {
        targetUser = state.data.users.find(u => u.email === meEmail) || state.data.users[0] || null;
      }
      if (!targetUser) {
        toast('No user available to issue a token for. Create a user first.', 'error');
        return;
      }
      // Patch openIssueToken's submit path to also remember the raw token.
      // Simpler: wrap fetch via api() — but openIssueToken does its own fetch.
      // Trick: monkey-patch toast-after-refresh by listening to refresh().
      // Cleanest path: just open the drawer; after the user copies, the
      // dropdown won't auto-substitute. Acceptable trade-off but spec wants
      // substitution, so we call the admin endpoint directly with a small
      // inline helper instead.
      openSetupIssueToken(targetUser);
    } else {
      // Quick self-issue.
      try {
        const r = await api('/api/me/tokens', {
          method: 'POST',
          body: JSON.stringify({ label: 'setup-quick' }),
        });
        const created = r.token;
        if (r.id != null && created) rememberRawToken(r.id, created);
        if (r.id != null) { activeTokenId = r.id; setupPreferredTokenId = r.id; }
        await refresh();
        toast('Token created — autofilled into the snippets below');
        // refresh() already triggered render(), but the new activeTokenId
        // we just set would be lost in the fresh renderSetup call. So we
        // also rely on default = tokens[0]. Acceptable: new tokens go to
        // the top of the list in most loaders. If not, the user can pick.
      } catch (e) {
        toast(e.message || 'Failed to create token', 'error');
      }
    }
  }

  // Lightweight wrapper around openIssueToken that captures the raw token
  // returned by the admin API and remembers it in sessionStorage so the
  // setup snippets get auto-substituted.
  function openSetupIssueToken(u) {
    let label = 'setup-quick';
    let capUsd = '';
    let capTokens = '';
    let createdToken = null;
    let createdId = null;

    const formBody = el('div', { class: 'field-group' },
      el('div', { class: 'field' },
        el('label', { class: 'field-label' }, 'Label'),
        el('input', { value: label, on: { input: (e) => label = e.target.value } }),
      ),
    ...
    async function submit() {
      try {
        ...
        createdToken = r.token;
        createdId = r.id ?? r.tokenId ?? null;
        if (createdId != null && createdToken) {
          rememberRawToken(createdId, createdToken);
          setupPreferredTokenId = createdId;
        }
        formBody.classList.add('hidden');
        revealBody.classList.remove('hidden');
        revealBody.replaceChildren(
          el('div', { class: 'alert alert--success' }, icon('check', 14),
            el('div', {}, el('div', { class: 'fw-600' }, 'Token created'),
              el('div', { class: 'fs-12 mt-1' }, 'Copy it now — you won\'t see it again. We also kept it in this browser tab\'s memory so the Setup snippets fill in automatically.'))),
          el('div', { class: 'code mt-2' }, createdToken,
            el('button', { class: 'btn btn--ghost btn--sm copy-btn', on: { click: () => copyToClipboard(createdToken) } },
              icon('copy', 14), 'Copy')),
        );
        primary.classList.add('hidden');
        if (createdId != null) { activeTokenId = createdId; setupPreferredTokenId = createdId; }
        await refresh();
      } catch (e) { toast(e.message, 'error'); }
    }
    const d = drawer({ title: 'Issue token', subtitle: `for ${u.email}`, body, footer });
  ...
  async function runTokenCheck() {
    ...
    if (!tok) {
      tokenCheckResult = { kind: 'amber', message: 'Paste or create a token first — the dropdown only knows the token prefix, not the secret.' };
      paint();
      return;
    }
    tokenCheckResult = { kind: 'pending' };
    paint();
    try {
      const r = await fetch(`${baseUrl}/v1/token/check`, {
        method: 'GET',
      ...
      if (r.ok && body?.ok) {
        tokenCheckResult = { kind: 'ok', body };
      } else {
        tokenCheckResult = { kind: 'fail', status: r.status, body };
      }
    } catch (e) {
      tokenCheckResult = { kind: 'fail', network: true, message: e?.message || 'Network error' };
    }
    paint();
  }
  ...
  function tokenCheckPanel() {
    if (!tokenCheckResult) {
      return el('div', { class: 'notice notice--info' },
        'Click ', el('strong', {}, 'Run token check'), ' to verify the selected token against ', el('code', { class: 'mono' }, '/v1/token/check'), '. No model request, no credits spent.');
    }
    if (tokenCheckResult.kind === 'pending') {
      return el('div', { class: 'notice notice--info' }, 'Checking token…');
    }
    if (tokenCheckResult.kind === 'amber') {
      return el('div', { class: 'alert alert--warn' }, icon('alert', 14), el('div', {}, tokenCheckResult.message));
    }
    if (tokenCheckResult.kind === 'ok') {
      const b = tokenCheckResult.body || {};
      const u = b.user || {};
      const t = b.token || {};
      return el('div', { class: 'alert alert--success' }, icon('check', 14),
        el('div', {},
          el('div', { class: 'fw-600' }, '✅ Token works'),
          el('div', { class: 'fs-12 mt-1' },
            (u.email || 'user'), ' · ', (u.role || 'developer'),
            ' · token ', el('code', { class: 'mono' }, t.label || t.prefix || `#${t.id ?? ''}`)),
        ));
    }
    // fail
    const r = tokenCheckResult;
    const msg = r.network
      ? `Network error: ${r.message}`
      : `HTTP ${r.status} — ${r.body?.error || r.body?.detail || r.body?.raw || 'token rejected'}`;
    return el('div', { class: 'alert alert--danger' }, icon('alert', 14),
      el('div', {},
        el('div', { class: 'fw-600' }, '❌ Token rejected'),
        el('div', { class: 'fs-12 mt-1' }, msg, ' — ',
          el('a', { href: 'javascript:void(0)', on: { click: (e) => { e.preventDefault(); setSetupTab('troubleshoot'); } } }, 'See troubleshooting')),
      ));
  }
  ...
  function buildHero() {
    const tok = effectiveToken();
    const masked = tok ? tok.slice(0, 12) + '…' + tok.slice(-4) : null;

    const step1 = el('div', { class: 'setup-step' },
      el('div', { class: 'setup-step__num' }, '1'),
      el('div', { class: 'setup-step__body' },
        el('div', { class: 'setup-step__title' }, 'Pick (or create) a token'),
        el('div', { class: 'setup-step__sub fs-12 text-muted' },
          'Tokens authenticate every request through this gateway. The raw ',
          el('code', { class: 'mono' }, 'sp_*'), ' value is only shown once at creation — we keep it in this browser tab\'s memory to autofill the snippets below.'),
        el('div', { class: 'mt-2' }, tokenSelector()),
        tok
          ? el('div', { class: 'setup-token-preview mt-2' },
              el('span', { class: 'fs-12 text-subtle' }, 'Token: '),
              el('code', { class: 'mono setup-token-value' }, revealed ? tok : masked),
              el('button', {
                class: 'btn btn--ghost btn--sm',
                on: { click: () => { revealed = !revealed; paint(); } },
              }, revealed ? 'Hide' : 'Reveal'),
              el('button', {
                class: 'btn btn--ghost btn--sm',
                on: { click: () => copyToClipboard(tok) },
              }, icon('copy', 12), 'Copy'),
            )
          : el('div', { class: 'notice notice--warn mt-2' },
              'Active token\'s raw value isn\'t in this browser tab. The snippets below will keep ',
              el('code', { class: 'mono' }, '<YOUR_TOKEN>'), ' as a placeholder — replace it with the value you saved when the token was created, or create a new token here.'),
        tok ? el('div', { class: 'fs-12 text-subtle mt-2' },
          'Heads up: once you leave this page (close the tab or sign out), the full token won\'t be shown again.') : null,
      ),
    );

    const agentPrompt = buildAgentSetupPrompt(baseUrl, null, tokenForSnippet());
    const tellAgent = el('div', { class: 'setup-step setup-step--agent' },
      el('div', { class: 'setup-step__num' }, icon('plug', 16)),
      el('div', { class: 'setup-step__body' },
        el('div', { class: 'setup-step__title' }, 'Tell an agent to set itself up'),
        el('div', { class: 'setup-step__sub fs-12 text-muted' },
          'Paste this into OCPlatform, Claude Code, Codex CLI, Kimi CLI, or any LLM-driven agent. It will read the skill and wire the gateway.'),
        setupCodeBlock(agentPrompt),
      ),
    );

    const oneShot = buildOneShotScript(baseUrl, tokenForSnippet());
    const scriptToggle = el('details', { class: 'setup-script' },
      el('summary', {}, 'Show full script'),
      el('div', { class: 'code mt-2' }, oneShot,
        el('button', { class: 'btn btn--ghost btn--sm copy-btn', on: { click: () => copyToClipboard(oneShot) } },
          icon('copy', 14), 'Copy')),
    );
    const step2 = el('div', { class: 'setup-step' },
      el('div', { class: 'setup-step__num' }, '2'),
      el('div', { class: 'setup-step__body' },
        el('div', { class: 'setup-step__title' }, 'Run the one-shot setup on the OCPlatform machine'),
        el('div', { class: 'setup-step__sub fs-12 text-muted' },
          'Patches ', el('code', { class: 'mono' }, '~/.openclaw/openclaw.json'), ', ',
          el('code', { class: 'mono' }, 'models.json'), ', and ',
          el('code', { class: 'mono' }, 'auth-profiles.json'), ' (with backups), then restarts ',
          el('code', { class: 'mono' }, 'openclaw-gateway.service'), ' if present.'),
        el('div', { class: 'flex gap-2 mt-2', style: { flexWrap: 'wrap' } },
          el('button', { class: 'btn btn--primary btn--sm', on: { click: () => copyToClipboard(oneShot) } },
            icon('copy', 14), 'Copy one-shot setup'),
        ),
        scriptToggle,
      ),
    );

    const step3 = el('div', { class: 'setup-step' },
      el('div', { class: 'setup-step__num' }, '3'),
      el('div', { class: 'setup-step__body' },
        el('div', { class: 'setup-step__title' }, 'Verify the token reaches the gateway'),
        el('div', { class: 'setup-step__sub fs-12 text-muted' },
          'Calls ', el('code', { class: 'mono' }, 'GET /v1/token/check'), ' with the selected token. No model request, no spend.'),
        el('div', { class: 'flex gap-2 mt-2', style: { flexWrap: 'wrap' } },
          el('button', { class: 'btn btn--primary btn--sm', on: { click: runTokenCheck } },
            icon('check', 14), 'Run token check'),
        ),
        el('div', { class: 'mt-2' }, tokenCheckPanel()),
      ),
    );

    return el('div', { class: 'setup-hero__inner' },
      el('div', { class: 'setup-hero__head' },
        el('div', {},
          el('div', { class: 'setup-hero__eyebrow' }, 'Get started'),
          el('h2', { class: 'setup-hero__title' }, `Set up ${currentSetupClient() === 'hermes' ? 'Hermes' : 'OCPlatform'} in 3 steps`),
          el('div', { class: 'setup-hero__sub fs-12 text-muted' },
            'Gateway base URL · ', el('code', { class: 'mono' }, baseUrl)),
        ),
        el('div', { class: 'flex gap-2 items-center' },
          el('button', { class: 'btn btn--ghost btn--sm', on: { click: () => copyToClipboard(baseUrl) } },
            icon('copy', 14), 'Copy URL'),
        ),
      ),
      el('ol', { class: 'setup-steps-v2' }, step1, tellAgent, step2, step3),
    );
  }

  // ─── Tabs ─────────────────────────────────────────────────────
  function buildClientToggle() {
    const active = currentSetupClient();
    return el('div', { class: 'setup-client-toggle flex gap-2 items-center', style: { marginBottom: '8px' } },
      el('span', { class: 'fs-12 text-muted' }, 'Client:'),
      el('div', { class: 'tabs setup-client-tabs', role: 'tablist', 'aria-label': 'Agent client' },
        ...SETUP_CLIENTS.map(c =>
          el('button', {
            class: 'tab' + (c.id === active ? ' active' : ''),
            role: 'tab',
            'aria-selected': c.id === active ? 'true' : 'false',
            on: { click: () => { setSetupClient(c.id); paint(); } },
          }, c.label)
        ),
      ),
    );
  ...
  function buildTabs() {
    const active = currentSetupTab();
    const strip = el('div', { class: 'tabs setup-tabs', role: 'tablist', 'aria-label': 'Provider configuration' },
      ...SETUP_TABS.map(t =>
        el('button', {
          ...
          id: `setup-tab-${t.id}`,
          on: { click: () => { setSetupTab(t.id); paint(); } },
        }, t.label)
      ),
    );

    const panel = el('div', {
      class: 'setup-tab-panel',
      role: 'tabpanel',
      id: `setup-panel-${active}`,
      'aria-labelledby': `setup-tab-${active}`,
    });

    const client = currentSetupClient();
    const ctx = { baseUrl, token: tokenForSnippet(), tokenIsReal: !!effectiveToken(), client, tabId: active };
    if (client === 'hermes' && HERMES_PROVIDERS[active]) {
      // Hermes: render the YAML custom_providers wiring for this provider.
      const verifyCurl = active === 'anthropic'
        ? `curl -sS -i \\
  -H "Authorization: Bearer ${ctx.token}" \\
  -H "Content-Type: application/json" \\
  -H "anthropic-version: 2023-06-01" \\
  -d '{"model":"claude-haiku-4-5","max_tokens":16,"messages":[{"role":"user","content":"pong"}]}' \\
  ${baseUrl}/v1/messages`
        : `curl -sS -i \\
  -H "Authorization: Bearer ${ctx.token}" \\
  ${baseUrl}/v1/token/check`;
      panel.appendChild(hermesSetupBlock({
        tabId: active, baseUrl, token: ctx.token,
        agentLine: `Read ${baseUrl}/skills/hermes/SKILL.md and set me up for ${HERMES_PROVIDERS[active].name}. My sp_* token is ${ctx.token}.`,
        verifyCurl,
        verifyNote: active === 'anthropic'
          ? 'Should return HTTP/2 200 with the assistant text and an x-gateway-account header.'
          : 'Should return HTTP/2 200 with {"ok":true} — confirms the token before you wire the provider.',
      }));
    } else {
      const builder = TAB_BUILDERS[active] || TAB_BUILDERS.anthropic;
      panel.appendChild(builder(ctx));
    }

    return el('div', {}, buildClientToggle(), strip, panel);
  }

  function paint() {
    heroMount.replaceChildren();
    heroMount.appendChild(buildHero());
    tabsMount.replaceChildren();
    tabsMount.appendChild(buildTabs());
  }
...
/* ─── Snippet helpers reused by every tab ──────────────────── */
function setupCodeBlock(content) {
  return el('div', { class: 'code mt-2' }, content,
    el('button', { class: 'btn btn--ghost btn--sm copy-btn', on: { click: () => copyToClipboard(content) } },
      icon('copy', 14), 'Copy'));
}
function buildAgentSetupPrompt(_baseUrl, slug, token) {
  const skillPath = slug ? `/skills/${slug}/SKILL.md` : '/skills/SKILL.md';
  return `Read http://localhost:8080${skillPath} and set me up. My sp_* token is ${token}.`;
}
function setupAgentCard(agentLine) {
  if (!agentLine) return null;
  return el('div', { class: 'card setup-agent-card' },
    el('div', { class: 'setup-block-label' }, 'Tell an agent'),
    el('p', { class: 'fs-12 text-muted', style: { lineHeight: '1.5' } },
      'Paste this into another LLM-driven agent so it can read the provider skill and wire itself.'),
    setupCodeBlock(agentLine),
  );
}
function setupProviderBlock({ desc, providerJson, authProfile, verifyCurl, verifyNote, extra, agentLine }) {
  return el('div', { class: 'stack' },
    setupAgentCard(agentLine),
    desc ? el('p', { class: 'fs-13 text-muted', style: { lineHeight: '1.55' } }, desc) : null,
    el('div', { class: 'setup-block-label' }, 'Provider block — merge into ', el('code', { class: 'mono' }, 'openclaw.json'), ' and ', el('code', { class: 'mono' }, 'models.json')),
    setupCodeBlock(providerJson),
    authProfile ? el('div', { class: 'setup-block-label mt-3' }, 'Auth profile — merge into ', el('code', { class: 'mono' }, 'auth-profiles.json')) : null,
    authProfile ? setupCodeBlock(authProfile) : null,
    extra || null,
    verifyCurl ? el('div', { class: 'setup-block-label mt-3' }, 'Verify') : null,
    verifyNote ? el('p', { class: 'fs-12 text-muted', style: { lineHeight: '1.5' } }, verifyNote) : null,
    verifyCurl ? setupCodeBlock(verifyCurl) : null,
  );
}
...
const TAB_BUILDERS = {
  ...
  "profiles": {
    ...
    const verifyCurl = `curl -sS -i \\
  -H "Authorization: Bearer ${token}" \\
  -H "Content-Type: application/json" \\
  -H "anthropic-version: 2023-06-01" \\
  -d '{"model":"claude-sonnet-4-5-20250929","max_tokens":16,"messages":[{"role":"user","content":"pong"}]}' \\
  ${baseUrl}/v1/messages`;
    return setupProviderBlock({
      desc: 'Routes /v1/messages and /v1/messages/count_tokens through this gateway. authHeader: true forces Bearer instead of x-api-key — that\'s what the gateway expects.',
      providerJson, authProfile, verifyCurl,
      agentLine: buildAgentSetupPrompt(baseUrl, 'anthropic', token),
      verifyNote: 'Should return HTTP/2 200 with the assistant text and an x-gateway-account header.',
    });
  },
  ...
  ${baseUrl}/v1/glm/v1/messages`;
    return setupProviderBlock({
      desc: 'GLM is an Anthropic-format (Anthropic Messages) provider — a transparent passthrough to the z.ai GLM Coding Plan. It behaves exactly like the Anthropic tab: Anthropic SDKs append /v1/messages to the baseUrl ' + baseUrl + '/v1/glm, so the canonical client path is ' + baseUrl + '/v1/glm/v1/messages (the short alias ' + baseUrl + '/v1/glm/messages also works). authHeader: true forces Bearer; the gateway also accepts x-api-key. Default model glm-5.2 has native 1M context / 128K max output.',
      providerJson, authProfile, verifyCurl,
      agentLine: buildAgentSetupPrompt(baseUrl, 'glm', token),
      verifyNote: 'Expect HTTP/2 200 with x-gateway-provider: glm. If you get model_not_allowed_for_user, enable GLM for that user first. Valid models: glm-5.2, glm-5.1, glm-5, glm-5-turbo, glm-4.7, glm-4.6, glm-4.5 (there is no -1m / [1m] id variant — those 400).',
    });
  },
  ...
  "profiles": {
    ...
    const verifyCurl = `curl -sS -N \\
  -H "Authorization: Bearer ${token}" \\
  -H "Content-Type: application/json" \\
  -d '{"model":"gpt-5.5","stream":true,"input":[{"role":"user","content":[{"type":"input_text","text":"pong"}]}]}' \\
  ${baseUrl}/v1/responses`;
    return setupProviderBlock({
      desc: 'Use the openai-codex/gpt-* PI route for Super Proxy. Do NOT use openai/gpt-* + agentRuntime.id: "codex" for gateway-routed traffic — native Codex talks to ChatGPT directly and bypasses this gateway. The /v1 suffix on baseUrl is required and api must be "openai-responses".',
      providerJson, authProfile, verifyCurl,
      agentLine: buildAgentSetupPrompt(baseUrl, 'openai-codex', token),
      verifyNote: 'Returns an SSE stream of Responses API events. The first event should arrive within a second.',
    });
  },

  images({ baseUrl, token }) {
    const providerJson = `{
  "models": {
    "providers": {
      "openai": {
  ...
  ${baseUrl}/v1/images/generations`;
    return setupProviderBlock({
      desc: 'gpt-image-2 routes through the regular openai provider (NOT openai-codex). The /v1 suffix is required so calls land on /v1/images/generations and /v1/images/edits. If the gateway has no real OpenAI key, Super Proxy falls back to Codex/ChatGPT OAuth via the Responses API image_generation tool — the response shape stays { data: [{ b64_json }] }.',
      providerJson, authProfile, verifyCurl,
      agentLine: buildAgentSetupPrompt(baseUrl, 'openai-images', token),
      verifyNote: 'Look for x-gateway-image-mode in the response headers. Success = HTTP/2 200 with base64 image data.',
    });
  },
  ...
  ${baseUrl}/v1/groq/chat/completions`;
    return setupProviderBlock({
      desc: 'OpenAI-compatible Groq chat and embeddings under /v1/groq.',
      providerJson, authProfile, verifyCurl,
      agentLine: buildAgentSetupPrompt(baseUrl, 'groq', token),
      verifyNote: 'Expect HTTP/2 200 with a choices array.',
    });
  },

  cerebras({ baseUrl, token }) {
    const providerJson = `{
  "models": {
    "providers": {
      "cerebras": {
        "baseUrl": "${baseUrl}/v1/cerebras",
        "apiKey": "${token}",
        "models": [
  ...
  ${baseUrl}/v1/cerebras/chat/completions`;
    return setupProviderBlock({
      desc: 'OCPlatform — Cerebras. OpenAI-compatible Cerebras chat and embeddings under /v1/cerebras.',
      providerJson, authProfile, verifyCurl,
      agentLine: buildAgentSetupPrompt(baseUrl, 'cerebras', token),
      verifyNote: 'Expect HTTP/2 200 with a choices array.',
    });
  },
  ...
  ${baseUrl}/v1/kimi/chat/completions`;
    const anthropicCurl = `curl -sS -i \\
  -H "Authorization: Bearer ${token}" \\
  -H "Content-Type: application/json" \\
  -H "anthropic-version: 2023-06-01" \\
  -d '{"model":"k3","max_tokens":16,"messages":[{"role":"user","content":"pong"}]}' \\
  ${baseUrl}/v1/kimi/messages`;
    return setupProviderBlock({
      desc: 'Two shapes under /v1/kimi: OpenAI-compatible /chat/completions (kimi provider) and Anthropic-compatible /messages (kimi-anthropic provider).',
      providerJson, authProfile, verifyCurl,
      agentLine: buildAgentSetupPrompt(baseUrl, 'kimi', token),
      verifyNote: 'OpenAI-style chat completion. The Anthropic-style endpoint uses /v1/kimi/messages and the same Bearer token.',
      extra: el('div', { class: 'mt-3' },
        el('div', { class: 'setup-block-label' }, 'Anthropic-shape verify'),
        setupCodeBlock(anthropicCurl)),
    });
  },

  gemini({ baseUrl, token }) {
    // Primary use: pooled free-tier embeddings for OpenClaw memorySearch (8 keys -> rotation + failover).
    const memoryJson = `{
  "agents": {
    "main": {
      "memorySearch": {
  ...
  "models": {
    ...
    const embedCurl = `curl -sS -i \\
  -H "Authorization: Bearer ${token}" \\
  -H "Content-Type: application/json" \\
  -d '{"model":"gemini-embedding-2","input":"pong"}' \\
  ${baseUrl}/v1/gemini/embeddings`;
    const chatCurl = `curl -sS -i \\
  -H "Authorization: Bearer ${token}" \\
  -H "Content-Type: application/json" \\
  -d '{"model":"gemini-3.1-flash-lite","messages":[{"role":"user","content":"pong"}]}' \\
  ${baseUrl}/v1/gemini/chat/completions`;
    return setupProviderBlock({
      desc: 'Native Gemini free-tier pool (separate from OpenRouter\'s Gemini). Three OpenAI-compatible routes under /v1/gemini: /embeddings, /chat/completions, /tts. The main use is pooled embeddings for OCPlatform memorySearch across multiple free-tier keys (rotation + 429 failover). Distinct from the openrouter provider above.',
      providerJson, authProfile: memoryJson, verifyCurl: embedCurl,
      agentLine: buildAgentSetupPrompt(baseUrl, 'gemini', token),
      verifyNote: 'Embeddings return an OpenAI-shape {data:[{embedding:[...3072 floats]}]}. The authProfile block above is the memorySearch config (not an auth profile) — wire it into agents.main.memorySearch.',
      extra: el('div', { class: 'mt-3' },
        el('div', { class: 'setup-block-label' }, 'Chat verify (gemini-3.1-flash-lite)'),
        setupCodeBlock(chatCurl)),
    });
  },


  openrouter({ baseUrl, token }) {
    const providerJson = `{
  "models": {
    "providers": {
      "openrouter": {
  ...
  ${baseUrl}/v1/openrouter/chat/completions`;
    return setupProviderBlock({
      desc: 'OpenRouter is exposed as a strict Gemini-only provider under /v1/openrouter. Non-admin users are disabled by default; enable OpenRouter per user in Model access when needed.',
      providerJson, authProfile, verifyCurl,
      agentLine: buildAgentSetupPrompt(baseUrl, 'openrouter', token),
      verifyNote: 'Expect HTTP/2 200 with a choices array. If you get model_not_allowed_for_user, enable OpenRouter for that user first.',
    });
  },

  deepgram({ baseUrl, token }) {
    const providerJson = `{
  "models": {
    "providers": {
      "deepgram": {
  ...
  ${baseUrl}/v1/deepgram/listen?model=nova-3&smart_format=true`;
    const rawAudioCurl = `curl -sS -i \
  -H "Authorization: Bearer ${token}" \
  -H "Content-Type: audio/wav" \
  --data-binary @your-audio.wav \
  ${baseUrl}/v1/deepgram/listen?model=nova-3&smart_format=true`;
    return setupProviderBlock({
      desc: 'Deepgram transcription is available under /v1/deepgram/listen. It supports JSON URL payloads and raw audio uploads. Unknown Deepgram models are blocked before upstream.',
      providerJson, authProfile, verifyCurl,
      agentLine: buildAgentSetupPrompt(baseUrl, 'deepgram', token),
      verifyNote: 'Expect HTTP/2 200 with Deepgram results.metadata.duration and results.channels[].alternatives[].transcript.',
      extra: el('div', { class: 'mt-3' },
        el('div', { class: 'setup-block-label' }, 'Raw audio upload verify'),
        setupCodeBlock(rawAudioCurl)),
    });
  },
  ...
  -H "Authorization: Bearer ${token}" \
  -H "Content-Type: application/json" \
  -d '{"model":"grok-4.3","input":"pong"}' \
  ${baseUrl}/v1/xai/responses`;
    return setupProviderBlock({
      desc: 'xAI Grok is exposed through /v1/xai/responses, including Agent Tools such as web_search via the Responses tools array. Grok Voice uses /v1/xai/realtime/client_secrets, /v1/xai/tts, and /v1/xai/stt. Grok Imagine image/video generation lives under /v1/xai/images/* and /v1/xai/videos/*. Non-admin users are disabled by default; enable xAI per user in Model access when needed.',
      providerJson, authProfile, verifyCurl,
      agentLine: buildAgentSetupPrompt(baseUrl, 'xai', token),
      verifyNote: 'Expect HTTP/2 200 with x-gateway-provider: xai. If you get model_not_allowed_for_user, enable xAI for that user first.',
    });
  },

  runpod({ baseUrl, token }) {
    const providerJson = `{
  "models": {
    "providers": {
      "runpod": {
  ...
  -H "Authorization: Bearer ${token}" \
  -H "Content-Type: application/json" \
  -d '{"model":"qwen36-27b-fast","messages":[{"role":"user","content":"pong"}],"max_tokens":16}' \
  ${baseUrl}/v1/runpod/chat/completions`;
    const modelsCurl = `curl -sS \
  -H "Authorization: Bearer ${token}" \
  ${baseUrl}/v1/runpod/models`;
    return setupProviderBlock({
      desc: 'Runpod Serverless Qwen is exposed as OpenAI-compatible chat under /v1/runpod. Non-admin users are disabled by default; enable Runpod per user in Model access when needed.',
      providerJson, authProfile, verifyCurl,
      agentLine: buildAgentSetupPrompt(baseUrl, 'runpod', token),
      verifyNote: 'Expect HTTP/2 200 with x-gateway-provider: runpod. If you get model_not_allowed_for_user, enable Runpod for that user first.',
      extra: el('div', { class: 'mt-3' },
        el('div', { class: 'setup-block-label' }, 'List models'),
        setupCodeBlock(modelsCurl)),
    });
  },

  aside({ baseUrl, token }) {
    const taskCurl = `curl -sS -i \
  -H "Authorization: Bearer ${token}" \
  -H "Content-Type: application/json" \
  -d '{"prompt":"Open Gmailnator, create a temporary Gmail address, and return only the email address.","timeoutMs":600000}' \
  ${baseUrl}/api/unsupported/aside/task`;
    const mcpEndpoint = `${baseUrl}/api/unsupported/aside/mcp`;
    const taskBody = `{
  "prompt": "Describe the browser task to run in your assigned Aside profile.",
  "timeoutMs": 600000,
  "useExec": false,
  ...
  "effort": "minimal"
    ...
    return el('div', { class: 'stack' },
      el('p', { class: 'fs-13 text-muted', style: { lineHeight: '1.55' } },
        'Aside runs browser automation on your assigned Mac profile through Super Proxy. Clients authenticate with their normal ',
        el('code', { class: 'mono' }, 'sp_*'),
        ' API token; Super Proxy resolves the user assignment and forwards to the right Mac gateway server-side.'),
      el('div', { class: 'setup-block-label' }, 'Task endpoint'),
      setupCodeBlock(`POST ${baseUrl}/api/unsupported/aside/task`),
      el('p', { class: 'fs-12 text-muted', style: { lineHeight: '1.5' } },
        'Use for one-shot browser tasks. Required field: ', el('code', { class: 'mono' }, 'prompt'),
        '. Optional fields: ', el('code', { class: 'mono' }, 'timeoutMs'), ', ',
        el('code', { class: 'mono' }, 'useExec'), ', ', el('code', { class: 'mono' }, 'model'), ', ',
        el('code', { class: 'mono' }, 'provider'), ', ', el('code', { class: 'mono' }, 'speed'), ', ',
        el('code', { class: 'mono' }, 'effort'), '.'),
      el('div', { class: 'setup-block-label mt-3' }, 'Task request body'),
      setupCodeBlock(taskBody),
      el('div', { class: 'setup-block-label mt-3' }, 'Task curl example'),
      setupCodeBlock(taskCurl),
      el('div', { class: 'setup-block-label mt-3' }, 'MCP endpoint'),
      setupCodeBlock(mcpEndpoint),
      el('p', { class: 'fs-12 text-muted', style: { lineHeight: '1.5' } },
        'Point Streamable HTTP MCP clients at this Super Proxy URL with the same Bearer token. Super Proxy owns the per-user Mac routing.'),
      el('div', { class: 'notice notice--warn mt-3' },
        el('strong', {}, 'Do not call Mac gateway URLs directly from clients.'),
        ' Mac endpoints such as ', el('code', { class: 'mono' }, '/v1/profiles'), ', ',
        el('code', { class: 'mono' }, '/v1/profiles/:label/tasks'), ', and ',
        el('code', { class: 'mono' }, '/v1/profiles/:label/mcp'),
        ' are admin/internal surfaces used by Super Proxy with stored gateway service tokens.'),
    );
  },

  manual({ baseUrl, token }) {
  ...
  "models": {
    ...
    const restart = 'systemctl restart openclaw-gateway.service';
    return el('div', { class: 'stack' },
      el('p', { class: 'fs-13 text-muted', style: { lineHeight: '1.55' } },
        'Paste this complete bundle into ',
        el('code', { class: 'mono' }, '~/.openclaw/openclaw.json'), ' and ',
        el('code', { class: 'mono' }, '~/.openclaw/agents/main/agent/models.json'),
        ', then drop the matching profiles into ',
        el('code', { class: 'mono' }, '~/.openclaw/agents/main/agent/auth-profiles.json'), '.'),
      el('div', { class: 'setup-block-label' }, 'Complete provider bundle'),
      setupCodeBlock(providerBundle),
      el('div', { class: 'setup-block-label mt-3' }, 'Restart the gateway'),
      setupCodeBlock(restart),
      el('div', { class: 'notice notice--info mt-3' },
        'Pass an ', el('code', { class: 'mono' }, 'x-conversation-id'), ' header to keep a single conversation pinned to the same upstream account — better prompt-cache reuse, lower spend.'),
    );
  },

  fusion({ baseUrl, token }) {
    const desc = 'Fusion sends your prompt to multiple models in parallel, then a synthesizer compares their responses and writes a better final answer. No extra provider config needed — fusion works through the same sp_* token you already have.';

    const presetsTable = el('table', { class: 'tbl tbl-collapse', style: { fontSize: '12px' } },
      el('thead', {}, el('tr', {},
        el('th', {}, 'Model alias'),
        el('th', {}, 'Panel models'),
        el('th', {}, 'Synthesizer'),
      ...
      el('tbody', {},
        el('tr', {},
          el('td', {}, el('code', { class: 'mono' }, 'fusion/max')),
          el('td', { class: 'text-muted' }, 'Claude Opus 4.8 + GPT-5.5 + Tencent HY3 Free'),
          el('td', { class: 'text-muted' }, 'Claude Opus 4.8'),
          el('td', { class: 'text-muted' }, '~$0.20\u2013$0.50+'),
        ),
        el('tr', {},
          el('td', {}, el('code', { class: 'mono' }, 'fusion/quality')),
          el('td', { class: 'text-muted' }, 'Claude Opus 4.8 + GPT-5.5 + Tencent HY3 Free'),
          el('td', { class: 'text-muted' }, 'Claude Opus 4.8'),
          el('td', { class: 'text-muted' }, '~$0.10\u2013$0.30'),
        ),
        el('tr', {},
          el('td', {}, el('code', { class: 'mono' }, 'fusion/budget')),
          el('td', { class: 'text-muted' }, 'Claude Sonnet + GPT-5.4 + Tencent HY3 Free'),
          el('td', { class: 'text-muted' }, 'Claude Sonnet'),
          el('td', { class: 'text-muted' }, '~$0.03\u2013$0.08'),
        ),
        el('tr', {},
          el('td', {}, el('code', { class: 'mono' }, 'fusion/custom')),
          el('td', { class: 'text-muted' }, 'Your choice (1\u20138)'),
          el('td', { class: 'text-muted' }, 'Your choice'),
          el('td', { class: 'text-muted' }, 'Varies'),
        ),
        el('tr', {},
          el('td', {}, el('code', { class: 'mono' }, 'fusion/<preset>')),
          el('td', { class: 'text-muted' }, 'Saved via dashboard'),
          el('td', { class: 'text-muted' }, 'Saved via dashboard'),
          el('td', { class: 'text-muted' }, 'Varies'),
        ),
      ),
    ...
    const synthesizeCurl = `curl -sS -N \\
  -H "Authorization: Bearer ${token}" \\
  -H "Content-Type: application/json" \\
  -d '{
  ...
  ${baseUrl}/v1/fusion/chat/completions`;

    const compareCurl = `curl -sS \\
  -H "Authorization: Bearer ${token}" \\
  -H "Content-Type: application/json" \\
  -d '{
    "model": "fusion/quality",
  ...
  ${baseUrl}/v1/fusion/chat/completions`;
    ...
    const modelsCurl = `curl -sS -H "Authorization: Bearer ${token}" \\
  ${baseUrl}/v1/models`;

    return el('div', { class: 'stack' },
      setupAgentCard(`Read ${baseUrl}/skills/fusion/SKILL.md and set me up for Fusion multi-model deliberation. My sp_* token is ${token}.`),
      el('p', { class: 'fs-13 text-muted', style: { lineHeight: '1.55' } }, desc),

      el('div', { class: 'setup-block-label' }, 'Available presets'),
      el('div', { class: 'table-wrap' }, presetsTable),

      el('div', { class: 'setup-block-label mt-3' }, 'Synthesize mode ', el('span', { class: 'text-muted fs-12' }, '\u2014 best answer from multiple models')),
      setupCodeBlock(synthesizeCurl),

      el('div', { class: 'setup-block-label mt-3' }, 'Compare mode ', el('span', { class: 'text-muted fs-12' }, '\u2014 see each model\u2019s raw response side-by-side')),
      setupCodeBlock(compareCurl),

      el('div', { class: 'setup-block-label mt-3' }, 'Custom panel ', el('span', { class: 'text-muted fs-12' }, '\u2014 pick your own models')),
      setupCodeBlock(customCurl),

      el('div', { class: 'setup-block-label mt-3' }, 'Model discovery'),
      el('p', { class: 'fs-12 text-muted', style: { lineHeight: '1.5' } },
        'Fusion presets appear in ', el('code', { class: 'mono' }, 'GET /v1/models'),
        ' so clients like Cursor can discover them. Your saved custom presets are included too.'),
      setupCodeBlock(modelsCurl),

      el('div', { class: 'setup-block-label mt-3' }, 'Custom presets'),
      el('p', { class: 'fs-12 text-muted', style: { lineHeight: '1.5' } },
        'Create and manage custom presets in the ',
        el('a', { href: '#fusion', style: { color: 'var(--accent)' } }, 'Fusion dashboard'),
        '. Saved presets are usable as ', el('code', { class: 'mono' }, 'model: "fusion/<name>"'),
        ' from any client.'),

      el('div', { class: 'setup-block-label mt-3' }, 'OCPlatform setup'),
      el('p', { class: 'fs-12 text-muted', style: { lineHeight: '1.5' } },
        'Fusion needs a provider block in ',
        el('code', { class: 'mono' }, 'openclaw.json'),
        ' / ',
        el('code', { class: 'mono' }, 'models.json'),
        ' with ', el('code', { class: 'mono' }, 'api: "openai-completions"'),
        ' and ', el('code', { class: 'mono' }, 'baseUrl'),
        ' ending in ', el('code', { class: 'mono' }, '/v1/fusion'),
        '. You also need to add fusion models to ',
        el('code', { class: 'mono' }, 'agents.defaults.models'),
        ' or they won\u2019t appear in ', el('code', { class: 'mono' }, '/models'),
        '. See the ', el('a', { href: `${baseUrl}/skills/fusion/SKILL.md`, style: { color: 'var(--accent)' } }, 'fusion skill guide'),
        ' for full instructions. The one-shot setup script above handles all of this automatically.'),
    );
  },

  troubleshoot() {
    const item = (q, a) => el('li', {},
      el('div', { class: 'setup-troubleshoot__q' }, q),
      el('div', { class: 'setup-troubleshoot__a fs-13 text-muted' }, a),
    );
    return el('ul', { class: 'setup-troubleshoot' },
      item(
        '401 “Invalid or disabled API token” when running the token check',
        el('span', {},
          'The token in your Authorization header doesn\'t match a live row in this gateway. Causes: you pasted the ',
          el('code', { class: 'mono' }, 'token_prefix'),
          ' shown in the dashboard (only the first ~12 chars) instead of the full ',
          el('code', { class: 'mono' }, 'sp_*'), ' secret saved at creation time; the token was disabled or deleted; or you typed it into the wrong field. Create a new token and re-run the check.'),
      ),
      item(
        '401 “invalid x-api-key” in usage logs',
        el('span', {},
          'OCPlatform is sending Anthropic-style ', el('code', { class: 'mono' }, 'x-api-key'),
          ' instead of Bearer. Set ', el('code', { class: 'mono' }, 'authHeader: true'),
          ' on the Anthropic (and ', el('code', { class: 'mono' }, 'kimi-anthropic'),
          ') provider block so OCPlatform uses ', el('code', { class: 'mono' }, 'Authorization: Bearer …'),
          '. The gateway accepts both, but Bearer is the canonical shape.'),
      ),
      item(
        '401 / 403 with “IP not allowed” or similar',
        'Your token has an IP allow-list on it and your machine isn\'t in the list. Either disable the IP restriction on the token in the dashboard, or issue a new unrestricted token for this machine.',
      ),
      item(
        'Codex still hits ChatGPT directly instead of the gateway',
        el('span', {},
          'The native Codex runtime in OCPlatform (',
          el('code', { class: 'mono' }, 'agentRuntime.id: "codex"'),
          ') talks straight to ChatGPT and ignores any HTTP base URL. To route through this gateway, use the ',
          el('code', { class: 'mono' }, 'openai-codex'), ' provider with ',
          el('code', { class: 'mono' }, 'api: "openai-responses"'), ' and ',
          el('code', { class: 'mono' }, 'baseUrl: <gateway>/v1'),
          ', and pick a ', el('code', { class: 'mono' }, 'openai-codex/*'), ' model in your agent.'),
      ),
      item(
        '“missing Authorization header” / connection refused on token check',
        'Your curl probably split lines wrong, or the URL doesn\'t include /v1/token/check. Copy the verify curl from the hero card above and re-run it verbatim.',
      ),
      item(
        'Image generation returns 404 or “unknown model”',
        el('span', {},
          'You configured ', el('code', { class: 'mono' }, 'openai-codex'),
          ' but not the regular ', el('code', { class: 'mono' }, 'openai'),
          ' provider. OCPlatform\'s image plugin uses the plain OpenAI provider for ',
          el('code', { class: 'mono' }, 'gpt-image-2'),
          ' — see the Images tab.'),
      ),
      item(
        'Upstream account doesn\'t stay sticky across turns',
        el('span', {},
          'Set an ', el('code', { class: 'mono' }, 'x-conversation-id'),
          ' header on every request in the same conversation. Without it the governor will load-balance freely and lose prompt-cache hits.'),
      ),
    );
  },
};

/* ─── One-shot setup script (Step 2) ───────────────────────── */
function buildOneShotScript(baseUrl, token) {
...
const glm = { baseUrl: baseUrl + '/v1/glm', api: 'anthropic-messages', authHeader: true, apiKey: token, models: [{ id: 'glm-5.2', name: 'GLM 5.2' }] };
const openrouter = { baseUrl: baseUrl + '/v1/openrouter', apiKey: token, models: [{ id: 'tencent/hy3:free', name: 'Tencent HY3 Free' }] };
const deepgram = { baseUrl: baseUrl + '/v1/deepgram', apiKey: token, models: [{ id: 'nova-3', name: 'Deepgram Nova-3' }, { id: 'nova-2', name: 'Deepgram Nova-2' }, { id: 'whisper', name: 'Deepgram Whisper Cloud' }] };
const xai = { baseUrl: baseUrl + '/v1/xai', api: 'openai-responses', apiKey: token, models: [{ id: 'grok-4.3', name: 'Grok 4.3', reasoning: true, input: ['text', 'image'] }] };
const runpod = { baseUrl: baseUrl + '/v1/runpod', apiKey: token, models: [{ id: 'qwen36-27b', name: 'Qwen3.6 27B' }, { id: 'qwen36-27b-fast', name: 'Qwen3.6 27B Fast' }] };
...
profiles['glm:manual'] = { type: 'token', provider: 'glm', token };
...
function renderMonitoring() {
  ...
  function paint(overview, series, cache, reliability, utilization, top, burn, budget) {
    ...
    poolsHost.replaceChildren(card('Pool health', poolTable(reliability), `${reliability.inFlightTotal} in flight`), card('Subscription utilization', utilTable(utilization.accounts)));
...
async function renderAside() {
  return el('div', { class: 'panel' },
    el('h2', { class: 'panel__title' }, 'Aside'),
    el('p', { class: 'muted' }, 'Aside browser-automation control plane is not included in Super Proxy OSS builds.')
  );
}
