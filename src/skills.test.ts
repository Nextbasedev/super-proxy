...
test('master skill is served as markdown', async () => {
  ...
  assert.match(res.body, /Nextbase Model Gateway/);
});

test('kimi skill mentions current Kimi models', async () => {
  const app = await skillsApp();
...
test('cerebras skill mentions current gpt-oss-120b fallback', async () => {
  const app = await skillsApp();
...
test('groq skill mentions chat and transcription routes', async () => {
  const app = await skillsApp();
...
test('xai skill documents Agent Tools web search', async () => {
  const app = await skillsApp();
...
test('hermes skill documents custom_providers + anthropic_messages + key_env', async () => {
  const app = await skillsApp();
  ...
  assert.match(res.body, /NEXTBASE_MODEL_GATEWAY_API_KEY/);
  assert.match(res.body, /nextbase-anthropic/);
});

test('master skill lists the Hermes client setup', async () => {
  const app = await skillsApp();
  const res = await app.inject('/skills/SKILL.md');
  assert.equal(res.statusCode, 200);
...
test('all provider skill files return 200', async () => {
  const app = await skillsApp();
