...
test('Fish TTS forwards s2.1-pro-free with Calm Male Narrator default', async () => {
  ...
  const res = await app.inject({ method: 'POST', url: '/v1/fish/tts', headers: { authorization: `Bearer ${token.raw}` }, payload: { input: 'Hello there' } });
  await app.close();
  globalThis.fetch = oldFetch;
  assert.equal(res.statusCode, 200, res.body);
  assert.equal(seenUrl, 'https://fish.test/v1/tts');
  assert.equal(seenHeaders.model, 's2.1-pro-free');
  assert.equal(seenBody.text, 'Hello there');
