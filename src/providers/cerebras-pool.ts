...
import { catalogDirectRuntimeModelIdsWithCapability } from './model-catalog.js';

export const DEFAULT_CEREBRAS_MODEL = 'gpt-oss-120b';
export const KNOWN_CEREBRAS_MODELS = new Set(catalogDirectRuntimeModelIdsWithCapability('cerebras', 'chat'));

export interface CerebrasLimit { account_id: number; model: string; rpm: number | null; rpd: number | null; tpm: number | null; tpd: number | null }
export interface CerebrasCounters { model: string; minuteRequests: number; minuteTokens: number; dayRequests: number; dayTokens: number; limit?: CerebrasLimit }
...
export function cerebrasWindows(now = new Date()): { minute: string; day: string } {
...
function readCounters(accountId: number, model: string, now = new Date()) {
  const { minute, day } = cerebrasWindows(now);
...
export function selectCerebrasAccountForModel(model: string, stickyKey: string, estimatedTokens = 0, excludeIds: number[] = []): ProviderAccount | null {
  const now = Date.now();
  ...
  const rows = getDb().prepare(`
    ...
    WHERE provider = 'cerebras' AND enabled = 1
  ...
  const eligible = rows.filter((a: any) => {
    ...
    const cd = getDb().prepare('SELECT cooldown_until FROM cerebras_model_cooldowns WHERE account_id=? AND model=?').get(a.id, model) as any;
    if ((cd?.cooldown_until || 0) > now) return false;
    const limit = getDb().prepare('SELECT account_id,model,rpm,rpd,tpm,tpd FROM cerebras_limits WHERE account_id=? AND model=?').get(a.id, model) as CerebrasLimit | undefined;
    if (limit) {
...
export function recordCerebrasRequest(account_id: number, model: string, tokens: number, ok: boolean): void {
  if (!ok) return;
  const { minute, day } = cerebrasWindows(new Date());
  getDb().prepare(`
    INSERT INTO cerebras_usage_buckets (account_id, model, window_minute, window_day, requests, tokens)
    VALUES (?, ?, ?, ?, 1, ?)
...
export function markCerebrasCooldown(account_id: number, model: string, ms: number, reason: string): void {
  const until = Date.now() + Math.max(0, ms);
  getDb().prepare(`
    INSERT INTO cerebras_model_cooldowns (account_id, model, cooldown_until, reason)
    VALUES (?, ?, ?, ?)
...
export function getCerebrasLiveCounters(account_id: number): CerebrasCounters[] {
  const models = new Set<string>();
  for (const r of getDb().prepare('SELECT model FROM cerebras_limits WHERE account_id=?').all(account_id) as any[]) models.add(r.model);
  for (const r of getDb().prepare('SELECT DISTINCT model FROM cerebras_usage_buckets WHERE account_id=? ORDER BY model').all(account_id) as any[]) models.add(r.model);
  for (const r of getDb().prepare('SELECT model FROM cerebras_model_cooldowns WHERE account_id=? AND cooldown_until > ?').all(account_id, Date.now()) as any[]) models.add(r.model);
  return [...models].sort().map((model) => {
    const limit = getDb().prepare('SELECT account_id,model,rpm,rpd,tpm,tpd FROM cerebras_limits WHERE account_id=? AND model=?').get(account_id, model) as CerebrasLimit | undefined;
