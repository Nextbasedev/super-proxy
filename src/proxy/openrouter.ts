...
import { DEFAULT_OPENROUTER_MODEL, KNOWN_OPENROUTER_MODELS, markOpenRouterCooldown, recordOpenRouterSuccess, selectOpenRouterAccount } from '../providers/openrouter-pool.js';
...
// OpenRouter returns HTTP 402 (Payment Required) when an account is out of
// credits. Account-wide, retryable on another pool account.
export function isOpenRouterInsufficientCredits(status: number, body: string): boolean {
  if (status === 402) return true;
  return status === 403 && /insufficient.?credits?/i.test(body || '');
}
...
async function forwardOpenRouter(req: any, reply: any, endpoint: '/chat/completions') {
  ...
  const model = rawModel || DEFAULT_OPENROUTER_MODEL;

  // OpenRouter is intentionally strict: only models shown in our allowlist are
  // usable. Unknown/future model names must not silently route and create cost.
  if (!KNOWN_OPENROUTER_MODELS.has(model)) {
    reply.code(400).send(modelNotAllowedForUserError(`Model ${model} is not allowed for OpenRouter`));
    return;
  }

  const allowed = isModelAllowedForUser(auth.user, 'openrouter', model);
  if (!allowed.ok) {
  ...
  const limit = checkLooseLimit(auth.user, 'openrouter', auth.token, model);
  if (!limit.ok) {
  ...
  const tried: number[] = [];
  let lastError = 'No OpenRouter account available';
  let retryAfter = 60;

  for (let attempt = 0; attempt < 5; attempt++) {
    const account = selectOpenRouterAccount(stickyKey, tried);
    if (!account) break;
    tried.push(account.id);
    try {
      const upstream = await fetch(`${config.openrouterUpstreamUrl}${endpoint}`, {
        method: 'POST',
        headers: {
          authorization: `Bearer ${account.secret}`,
          'content-type': 'application/json',
          accept: stream ? 'text/event-stream' : 'application/json',
          'http-referer': process.env.OPENROUTER_HTTP_REFERER || process.env.GATEWAY_PUBLIC_URL || 'http://localhost',
          'x-title': process.env.OPENROUTER_APP_TITLE || process.env.GATEWAY_NAME || 'Super Proxy',
        },
        body: JSON.stringify(body),
        signal: AbortSignal.timeout(REQUEST_TIMEOUT_MS),
      });
      reply.header('x-gateway-provider', 'openrouter');
      ...
      if (upstream.status === 429) {
        ...
        markOpenRouterCooldown(account.id, ms, `rate limited (${upstream.status})`);
        req.log?.info?.({ provider: 'openrouter', account: account.label, model, attempt: attempt + 1 }, 'OpenRouter rate limited; switching to another account');
        recordUsage({ userId: auth.user.id, tokenId: auth.token.id, providerAccountId: account.id, provider: 'openrouter', endpoint: `/v1/openrouter${endpoint}`, model, stream, statusCode: upstream.status, inputTokens: inputEstimate, outputTokens: 0, estimatedCostUsd: 0, latencyMs: Date.now() - started, error: text.slice(0, 500) || 'rate limited', ...compressionFields(req), tokenLabel: auth.token.label, providerAccountLabel: account.label });
      ...
      if (upstream.status >= 400) {
        ...
        if (isOpenRouterInsufficientCredits(upstream.status, text)) {
          const cooldownMs = 6 * 60 * 60 * 1000; // 6h — credits need manual top-up
          markOpenRouterCooldown(account.id, cooldownMs, 'insufficient_credits');
          req.log?.info?.({ provider: 'openrouter', account: account.label, model, attempt: attempt + 1 }, 'OpenRouter account out of credits; switching to another account');
          recordUsage({ userId: auth.user.id, tokenId: auth.token.id, providerAccountId: account.id, provider: 'openrouter', endpoint: `/v1/openrouter${endpoint}`, model, stream, statusCode: upstream.status, inputTokens: inputEstimate, estimatedCostUsd: 0, latencyMs: Date.now() - started, error: lastError, ...compressionFields(req), tokenLabel: auth.token.label, providerAccountLabel: account.label });
          retryAfter = Math.max(retryAfter, 60);
          continue;
        }
        if (isModelUnavailableError(upstream.status, text)) {
          const clean = modelUnavailableError('openrouter', model);
      ...
      if (stream && upstream.body) {
        writeRawResponseHead(reply, upstream.status, { 'content-type': upstream.headers.get('content-type') || 'text/event-stream', 'cache-control': 'no-cache', 'x-gateway-provider': 'openrouter', 'x-gateway-account': account.label });
        ...
        try {
          while (true) {
            ...
            const surfaced = surfaceOpenAiCompatStreamChunk('openrouter', chunk, surfaceState);
          ...
          forcedLogReason = isAbortTimeoutError(err) ? 'openrouter_timeout' : 'openrouter_stream_interrupted';
          const interrupted = surfaceOpenAiCompatError('openrouter', null, 'stream_interrupted', isAbortTimeoutError(err) ? timeoutMessage(timeoutSeconds(REQUEST_TIMEOUT_MS)) : undefined).appendSse!;
        ...
        if (!surfaceState.sawCompletion && !forcedLogReason) {
          forcedLogReason = 'openrouter_stream_interrupted';
          const interrupted = surfaceOpenAiCompatError('openrouter', null, 'stream_interrupted').appendSse!;
          assembled += interrupted;
          reply.raw.write(interrupted);
        }
        reply.raw.end();
        recordOpenRouterSuccess(account.id);
        const usageEventId = recordUsage({ userId: auth.user.id, tokenId: auth.token.id, providerAccountId: account.id, provider: 'openrouter', endpoint: `/v1/openrouter${endpoint}`, model, stream, statusCode: upstream.status, ...sseState.usage, ttftMs, retryCount: attempt, retryReason: attempt > 0 ? 'account_rotation' : undefined, estimatedCostUsd: estimateCost(model, sseState.usage, 'openrouter'), latencyMs: Date.now() - started, ...compressionFields(req), tokenLabel: auth.token.label, providerAccountLabel: account.label });
        enforceAfterUsage(auth.user, 'openrouter', auth.token, model);
      ...
      try {
        const parsed = JSON.parse(text);
        const surfaced = surfaceOpenAiCompatError('openrouter', parsed, 'non_stream');
      ...
      recordOpenRouterSuccess(account.id);
      const usageEventId = recordUsage({ userId: auth.user.id, tokenId: auth.token.id, providerAccountId: account.id, provider: 'openrouter', endpoint: `/v1/openrouter${endpoint}`, model, stream, statusCode: upstream.status, ...usage, ttftMs: nonStreamTtftMs, retryCount: attempt, retryReason: attempt > 0 ? 'account_rotation' : undefined, estimatedCostUsd: estimateCost(model, usage, 'openrouter'), latencyMs: Date.now() - started, ...compressionFields(req), tokenLabel: auth.token.label, providerAccountLabel: account.label });
      enforceAfterUsage(auth.user, 'openrouter', auth.token, model);
      ...
      lastError = timedOut ? timeoutMessage(timeoutSeconds(REQUEST_TIMEOUT_MS)) : transientNetworkMessage('openrouter');
      if (!timedOut) {
        markOpenRouterCooldown(account.id, 60_000, `network error: ${err?.message || err}`);
        req.log?.info?.({ provider: 'openrouter', account: account.label, model, attempt: attempt + 1, error: String(err?.message || err) }, 'OpenRouter network error; switching to another account');
      }
      const statusCode = timedOut ? 504 : 502;
      const usageEventId = recordUsage({ userId: auth.user.id, tokenId: auth.token.id, providerAccountId: account.id, provider: 'openrouter', endpoint: `/v1/openrouter${endpoint}`, model, stream, statusCode, inputTokens: inputEstimate, estimatedCostUsd: 0, latencyMs: Date.now() - started, error: lastError, ...compressionFields(req), tokenLabel: auth.token.label, providerAccountLabel: account.label });
      if (timedOut) {
        logRequestResponse({ usageEventId, userId: auth.user.id, requestBody: { forcedLogReason: 'openrouter_timeout', providerAccountLabel: account.label, attempt: attempt + 1, headers: req.headers, body }, responseText: JSON.stringify(openAiError(lastError, 'timeout', 'gateway_timeout')) });
  ...
  reply.header('x-gateway-provider', 'openrouter');
  reply.code(429).send(openAiError(`OpenRouter capacity unavailable: ${lastError}`, 'rate_limit_exceeded', 'rate_limit_exceeded'));
}

export function registerOpenRouterProxy(app: FastifyInstance) {
  app.post('/v1/openrouter/chat/completions', (req, reply) => forwardOpenRouter(req, reply, '/chat/completions'));
}
